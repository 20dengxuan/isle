  ...
  <div class="tab">
    <header>
       <div v-if="state.isSignIn">
           <span  class="van-ellipsis">早上好,{{state.signinInfos.email}}</span><span @click="logout()" > <i class="iconfont icon-tuichu"></i>退出</span>
       </div>

       <div v-else>
         <button @click="tologin()">登记注册</button>
       </div>
    </header>
...
import {
  reactive, ref, computed, defineComponent,watch
} from 'vue';
import { useRouter } from 'vue-router';
import i18n from '../../assets/script/i18n';
import { $apiCache } from '../../assets/script/api-cache';
import {deleteSessionStorage} from '../../assets/script/utils'
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const state = reactive({
      isSignIn: store.state.isSignIn || false,
      signinInfos:store.state.signinInfos || {},
    });
    ...
    watch(()=>store.state.isSignIn ,(newVal)=>{
       state.isSignIn = newVal
       state.signinInfos = store.state.signinInfos
    })
    

    const logout = ()=>{
      $apiCache({key:'signOut'}).then(res=>{
        if(!res.code){
          deleteSessionStorage('SIGNININFOS')
          store.commit(" SET_SIGNIN",{})
