<template>
  <div class="exhibits">
     ...
     <div v-if="showSearch" class="searchList">
         <van-icon @click="showSearch = false" :style="{position:'absolute',right:'10px',top:'10px'}" name="cross" />
         <p>展品类目 | <span>{{state.category === ''?'全部':state.category_son?state.category+'/'+state.category_son:state.category}}</span></p>
         <p>展品品牌 | <span>{{state.brand?state.brand:'全部'}}</span></p>
         <p>价格区间 | <span>{{form.price_start}} — {{form.price_end}}</span></p>
     </div>     
...
console.log(root)
   ...
   const toSearch = (e)=>{
    ...
    state.brand = e.state.brand_id
    state.category = e.state.category
    state.category_son =e.state.category_id


     form.brand_id = e.form.brand_id
     form.price_start = e.form.price_start
     form.price_end = e.form.price_end
     form.category_id = e.form.category_id
     state.list = []
     form.page = 0
     onLoad()
   }
