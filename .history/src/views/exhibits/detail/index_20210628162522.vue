<template>
  <div class="detail">
    <div class="h_img">
       <van-img v-if="state.detail.images" height='14.375rem' width="100%;" @click="imgShow()" :src="'//image-dev.3-e.cn/'+state.detail?.images[0]" />    
    </div>
    ...
    <div v-if="!state.detail.detail">暂无</div>
  </div>
</template>
  ...
  name:'detail',
   setup(){
    ...
    watch(()=>store.state.lang,(newVal)=>{
     ...
     const imgShow = ()=>{
       const images = state.detail.images.map(item=>{
           return '//image-dev.3-e.cn/'+item
