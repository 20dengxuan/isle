...
export default {
  ...
  setup() {
    ...
    const state = reactive({
      card: [],
      data:[
         {title:computed(()=>app.$t('menu.news')) ,dada:[]},
         {title:computed(()=>app.$t('menu.spread')),dada:[]},
         {title:computed(()=>app.$t('title.enterprises')),dada:[]},
         {title:computed(()=>app.$t('menu.industry')) ,dada:[]}
     ]
    });
   ...
   computed(async ()=>{
       $apiCache({key:'getNewsList',type:2},{lang:app.$i18n.global.locale,page:1,page_size:12}).then(res=>{
        console.log(res)
        state.data[0].data =  res.data.data
      })
    })
