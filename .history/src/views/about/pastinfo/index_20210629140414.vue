<template>
    <div>
      ...
      <div v-for="(c,index) in centent" :key="index" class="jiazi" :class="(index+1)%2 === 0 ?'fan':''" >
          <div><p>{{c.year}}</p></div>
          <div>
              <div></div>
              <div></div>
...
export default {
    ...
    mounted(){
        $apiCache({key:'getPastinfo',type:2},{lang:this.$store.state.lang}).then(res=>{
    ...
    setup(){
        ...
        const change = ()=>{
          lang.value === 'zh' ? lang.value = 'en' : lang.value = 'zh'
        }

        watchEffect(()=>{
               console.log(lang)
               $apiCache({key:'getPastinfo',type:2},{lang:lang}).then(res=>{
                centent.value = res.data
                console.log(res)
            })
        })
