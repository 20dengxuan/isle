<template>
<div ref="root">
    <van-field @update:model-value="update"  @focus="focus" @blur="blur" class="seach" v-model="value" :placeholder="action.data">
         <template v-slot:label>
            <van-popover v-model:show="showPopover" :actions="actions" @select="onSelect">
              <template #reference>
               {{action.value}} <van-icon size="0.875rem" name="arrow-down" />
              </template>
            </van-popover>
         </template>
         
       </van-field>
       <div  v-if="seachlist" class="lists">
            <div style="text-align:center;line-height:7.375rem" v-if="state.list.length === 0">
                抱歉找不到数据
            </div>
            <div v-else>
                <van-loading v-if="loading" type="spinner" />
                <p @click="todetail(id={id:l?.id,company_id:l.company_id})" v-for="(l,index) in state.list" :key="index">
                  {{l.name}}
                </p>
            </div>
       </div>
       </div>
</template>

<script>
import { ref ,reactive,watch,onMounted} from 'vue';
import {useStore} from 'vuex'
import {useRouter} from 'vue-router'
import {$apiCache} from '../../../assets/script/api-cache'
import {useClickAway} from '@vant/use'
export default {
  setup() {
    const value = ref('')
    const store = useStore()

    const state = reactive({
       exhibi:[],
       list:[]
    })

    const action = reactive({
        value:'搜展品',
        data:'输入展品名称'
    })
    const showPopover = ref(false);
    const seachlist = ref(false)
    const loading = ref(false)

    // 通过 actions 属性来定义菜单选项
    const actions = [
      { text: '搜展商' ,data:'搜寻本届展商名称获取'},
      { text: '搜展品',data:'输入展品名称' },
     
    ];
    const onSelect = (a) => {
      action.value = a.text
      action.data = a.data
      state.list = []
      value.value = ''
    };


    const focus = () =>{
      seachlist.value = true
    }

    const blur = () =>{
      
    }

    const update =async (str)=>{
      state.list = []
      if(str === '') return
      if(action.value === '搜展商'){
         state.exhibi.map(item=>{
          if(item.name.indexOf(str) != -1){
            state.list.push(item)
          }
        })
        console.log(state.list)
      }else if(action.value === '搜展品'){
        loading.value = true
        await gettitlelist(str)
        loading.value = false
      }
    }
  watch(()=>store.state.lang,(newVal)=>{
      getExhibitSelection(newVal)
  })

  //展品
  const gettitlelist = (keyword)=>{
    
    $apiCache({key:'gettitlelist'},{lang:store.state.lang,keyword}).then(res=>{
     state.list = res.data
     console.log(res)
    })
  }

  //所有展商
  const getExhibitSelection = (lang)=>{
    $apiCache({key:'getExhibitSelection',type:2},{lang:lang}).then(res=>{
      state.exhibi = res.data
      console.log(state.exhibi)
    })
  }

  const root = ref()
  useClickAway(root,()=>{
    seachlist.value = false
  })
  const router = useRouter()
  const todetail = (id)=>{
    if(id.id){
      console.log('id' + id.id)
      router.push({name:'detail',query:{id:id.id}})
    }else if(id.company_id){
      router.push({name:'dirDetail',query:{id:id.company_id}})
    }
  }
   
   onMounted(()=>{
     getExhibitSelection(store.state.lang)
   })

    return {
      value,
      actions,
      action,
      onSelect,
      showPopover,
      focus,
      blur,
      seachlist,
      update,state,
      loading,
      todetail,root
    };
  },
};
</script>

<style lang="less" scoped>
div{

 position: relative;
     .seach{
        border-radius:1rem;
        font-size:0.875rem;
        height:2.1rem;
        display: flex;
        align-items: center;
        
      }
      .lists{
          width:100%;
          height: 7.375rem;
          background:white;
          position: absolute;
          z-index:2;
          margin-top:0.625rem;
          overflow:auto;
          p{
            text-overflow: ellipsis;
            padding:0.3125rem;
            font-size:0.875rem;
            white-space: nowrap;
            overflow: hidden;
          }
        }
}
</style>